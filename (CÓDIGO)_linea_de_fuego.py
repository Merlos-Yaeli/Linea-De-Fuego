...
# =====================================================================
# 1.1 CONTROLES DE TECLADO (para la simulación de carrera)
# =====================================================================
# Durante la carrera, cada tecla se lee al instante, SIN necesidad de
# presionar ENTER, tanto en Windows (msvcrt) como en Mac/Linux (termios).
# Si el programa se ejecuta en un entorno sin teclado interactivo
# (por ejemplo, una terminal redirigida), se usa input() como respaldo
# para que el juego nunca se rompa (requerimiento de robustez).

CONTROLES = {
    "w": "Acelerar a fondo",
    "a": "Esquivar / girar a la izquierda",
    "d": "Esquivar / girar a la derecha",
    "s": "Frenar (más control, menos velocidad)",
    " ": "Usar NITRO BOOST (barra espaciadora)",
...
def mostrar_controles():
    ...
    print("    [W] Acelerar a fondo")
    print("    [A] / [D] Esquivar obstáculos (izquierda / derecha)")
    print("    [S] Frenar para tomar curvas cerradas")
    print("    [ESPACIO] Usar Nitro Boost")
    print("    [E] Usar Escudo de Plasma")
    print("    [Q] Usar Misil Teledirigido")
    print("    [X] Abandonar la carrera")
    print("    (No es necesario presionar ENTER, solo la tecla)\n")


def leer_tecla(mensaje=""):
    """
    Lee UNA sola tecla presionada por el jugador sin esperar ENTER.
    - Windows: usa el módulo msvcrt.
    - Mac/Linux: usa termios/tty para poner la terminal en modo 'raw'.
    - Si no hay una terminal interactiva disponible, cae de forma segura
      a input() (requiere ENTER) para que el juego jamás se detenga.
    """
    if mensaje:
    ...
    try:
        ...
        if not sys.stdin.isatty():
            raise OSError("Entrada estándar no interactiva")
        try:
            import msvcrt
            tecla_cruda = msvcrt.getch()
            tecla = tecla_cruda.decode("utf-8", errors="ignore")
        except ImportError:
            import termios
            import tty
            fd = sys.stdin.fileno()
        ...
        # Respaldo seguro: pide la tecla seguida de ENTER
        return input().strip().lower()[:1]


def buscar_herramienta(jugador, nombre_base):
    """Busca en el inventario del jugador una herramienta que coincida
    con el nombre base (ej. 'Nitro Boost'), sin importar el sufijo MK."""
    for herramienta in jugador.herramientas:
        if herramienta.nombre.startswith(nombre_base):
            return herramienta
    return None
...
class Nivel(ElementoDesbloqueable):
    ...
    def jugar(self, jugador, jefe):
        ...
        jugador o el rival crucen la línea de meta'). En cada vuelta el
        jugador presiona una tecla (ver CONTROLES) que decide su acción.
        """
        ...
        while jugador.progreso < meta and rival_progreso < meta:
            tecla = leer_tecla("    ¿Qué haces? [W/A/D/S/ESPACIO/E/Q/X]: ")
            avance_jugador = 0

            # --- Condicionales según la tecla presionada ---
            if tecla == "w":
                avance_jugador = random.randint(9, 14)
                print("    🏎 ¡Aceleras a fondo!")
            elif tecla in ("a", "d"):
                avance_jugador = random.randint(7, 11)
                lado = "izquierda" if tecla == "a" else "derecha"
                print(f"    ↔ Esquivas hacia la {lado} y mantienes el ritmo.")
            elif tecla == "s":
                avance_jugador = random.randint(3, 6)
                print("    🐢 Frenas para tomar la curva con más control.")
            elif tecla == " ":
                herramienta = buscar_herramienta(jugador, "Nitro Boost")
                if herramienta:
                    herramienta.usar(jugador)
                else:
                    print("    ❌ No tienes Nitro Boost en el inventario.")
                avance_jugador = random.randint(11, 16) + int(jugador.velocidad_bonus * 10)
            elif tecla == "e":
                herramienta = buscar_herramienta(jugador, "Escudo de Plasma")
                if herramienta:
                    herramienta.usar(jugador)
                else:
                    print("    ❌ No tienes Escudo de Plasma en el inventario.")
                avance_jugador = random.randint(5, 9)
            elif tecla == "q":
                herramienta = buscar_herramienta(jugador, "Misil Teledirigido")
                if herramienta:
                    herramienta.usar(jugador)
                    rival_progreso = max(0, rival_progreso - 6)
                else:
                    print("    ❌ No tienes Misil Teledirigido en el inventario.")
                avance_jugador = random.randint(5, 9)
            elif tecla == "x":
                print("    🚪 Abandonas la carrera.")
                return self.perder()
            else:
                avance_jugador = random.randint(4, 8)
                print("    ⚠ Tecla no reconocida. Avanzas a velocidad normal.")
...
class LineaDeFuego:
    ...
    def menu_principal(self):
        while True:
            ...
            print("6) Ver controles del teclado")
            print("7) Salir")
            op = pedir_opcion("Elige una opción: ", ["1", "2", "3", "4", "5", "6", "7"])
            if op is None:
                continue
            if op == "1" or op == "2":
                self.pantalla_mapas()
